"""
Tool Bridge for Application Layer

Provides the bridge between MCP tools and the application layer,
handling tool discovery, validation, and execution coordination.
"""
...
class AvailableToolInfo:
    ...
    server_type: str  # filesystem, git, codebase, devtools, exa
...
class ToolBridge:
    ...
    async def get_available_tools(self, refresh_cache: bool = False) -> List[AvailableToolInfo]:
        """
        ...
        if (not refresh_cache and
            self._tool_cache and
                (current_time - self._last_cache_update) < self._cache_ttl_seconds):
        ...
        try:
            ...
            for server_info in servers_info:
                ...
                if server_status != "connected":
                    self.logger.warning(
                        f"Server {server_id} not connected, skipping")
                    continue
            ...
            self.logger.info(f"Cached {len(tools_info)} tools from {
    ...
    def _extract_tool_parameters(self, tool: MCPTool) -> Dict[str, Any]:
        """Extract parameter schema from MCP tool."""
        try:
            ...
            self.logger.warning(
                f"Failed to extract parameters for tool {tool.name}: {e}")
            return {}
    ...
    async def execute_tool_request(self, request: ToolExecutionRequest) -> ToolExecutionResponse:
        """
        ...
        try:
            self.logger.info(f"Executing tool {request.tool_name} with params: {
                             request.parameters}")

            # Execute via tool client
            ...
            self.logger.error(
                f"Tool {request.tool_name} execution failed: {error_msg}")
    ...
    async def execute_tool_for_agent(self, tool_call: ToolCall) -> ToolCall:
        ...
        try:
            ...
            if not tool_call.server_id:
                tool_call.server_id = await self._find_server_for_tool(tool_call.tool_name)
                if not tool_call.server_id:
                    raise ValueError(
            ...
            self.logger.info(f"Agent tool {tool_call.tool_name} executed successfully in {
    ...
    async def get_tools_for_agent_type(self, agent_type: AgentType) -> List[AvailableToolInfo]:
        """
        ...
        if agent_type == AgentType.PLANNING:
            ...
            return [tool for tool in all_tools if tool.server_type in ["codebase", "filesystem"]]
    ...
    async def get_tool_usage_recommendations(
        ...
        tool_keywords = {
            "filesystem": ["file", "read", "write", "directory", "path", "save", "load", "create"],
            "git": ["git", "commit", "branch", "diff", "log", "status", "history", "version"],
            "codebase": ["code", "function", "class", "analyze", "structure", "definition", "reference"],
            "devtools": ["test", "lint", "format", "build", "install", "run", "check", "validate"],
            "exa": ["search", "web", "internet", "online", "crawl", "url", "website", "information"]
        }
    ...
    async def validate_tool_parameters(
        ...
        try:
            ...
            if not tool_info:
                return False, f"Tool '{tool_name}' not found. Available tools: {list(self._tool_cache.keys())}"
            ...
            if hasattr(tool_info, 'parameters') and tool_info.parameters:
                ...
                properties = schema.get('properties', {}) if isinstance(
                    schema, dict) else {}
                required = schema.get('required', []) if isinstance(
                    schema, dict) else []
                ...
                if unexpected_params:
                    self.logger.warning(f"Tool '{tool_name}' received unexpected parameters: {
                                        unexpected_params}")
    ...
    def _generate_usage_example(self, tool_name: str, properties: Dict[str, Any], required: List[str]) -> str:
    ...
    async def get_server_status(self) -> Dict[str, Any]:
        ...
        try:
            servers_info = await self.tool_client.get_all_servers()
            return {
                "connected_servers": len([s for s in servers_info if s.get("status") == "connected"]),
    ...
    def get_tools_by_server(self, server_id: str) -> List[AvailableToolInfo]:
        ...
        return [tool_info for tool_info in self._tool_cache.values()
