...
def is_command_safe(command: str) -> bool:
    ...
    for _, commands in SAFE_COMMANDS.items():
...
def run_command(command: str, cwd: Optional[str] = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(
            cmd_parts, cwd=cwd, capture_output=True, text=True, timeout=timeout
        )
...
@mcp.tool()
def run_tests(
    pattern: Optional[str] = None, file_path: Optional[str] = None, path: str = "."
) -> str:
    """
    ...
    if (
        os.path.exists(os.path.join(abs_path, "requirements.txt"))
        or os.path.exists(os.path.join(abs_path, "pyproject.toml"))
        or any(f.endswith(".py") for f in os.listdir(abs_path))
    ):
    ...
    for cmd in test_commands:
        ...
        if result["success"]:
            ...
            if "not found" not in result.get(
                "error", ""
            ) and "command not found" not in result.get("stderr", ""):
...
def lint_code(file_path: Optional[str] = None, path: str = ".") -> str:
    ...
    if os.path.exists(os.path.join(abs_path, "package.json")):
        # Check for ESLint
        eslint_config = any(
            os.path.exists(os.path.join(abs_path, f))
            for f in [
                ".eslintrc.js",
                ".eslintrc.json",
                ".eslintrc.yml",
    ...
    for cmd in lint_commands:
        ...
        if result["success"]:
            ...
            if "not found" in result.get(
                "error", ""
            ) or "command not found" in result.get("stderr", ""):
...
def format_code(file_path: Optional[str] = None, path: str = ".") -> str:
    ...
    for cmd in format_commands:
        ...
        if result["success"]:
            ...
            results.append(
                f"❌ {cmd}: {result.get('stderr', result.get('error', 'Unknown error'))}"
            )
...
def install_dependencies(path: str = ".") -> str:
    ...
    if os.path.exists(os.path.join(abs_path, "requirements.txt")):
        result = run_command(
            "pip install -r requirements.txt", cwd=abs_path, timeout=300
        )
        if result["success"]:
