...
def analyze_project(path: str = ".") -> str:
    ...
    try:
        ...
        if "package.json" in [
            os.path.basename(f) for f in structure["config_files"].keys()
        ]:
            project_type = "Node.js/JavaScript Project"
        elif any(
            "requirements.txt" in f or "pyproject.toml" in f
            for f in structure["config_files"].keys()
        ):
        ...
        if primary_tech:
            analysis.extend(
                ["Primary Technologies:", *[f"  • {tech}" for tech in primary_tech], ""]
            )
        ...
        if structure["config_files"]:
            analysis.extend(
                [
                    ...
                    *[
                        f"  • {file}: {tech}"
                        for file, tech in structure["config_files"].items()
                    ],
                    "",
                ]
            )
        ...
        top_dirs = [
            d for d in structure["directories"] if "/" not in d and "\\" not in d
        ]
        if top_dirs:
            analysis.extend(
                ["Top-level Directories:", *[f"  • {d}/" for d in sorted(top_dirs)], ""]
            )
    ...
    try:
        ...
        def build_tree(current_path: str, prefix: str = "", depth: int = 0):
            ...
            try:
                ...
                dirs = [
                    e for e in entries if os.path.isdir(os.path.join(current_path, e))
                ]
                files = [
                    e for e in entries if os.path.isfile(os.path.join(current_path, e))
                ]
                ...
                for i, dir_name in enumerate(dirs):
                    ...
                    build_tree(
                        os.path.join(current_path, dir_name), next_prefix, depth + 1
                    )
...
@mcp.tool()
def find_definition(
    symbol: str, file_path: Optional[str] = None, path: str = "."
) -> str:
    """
...
def explain_codebase(path: str = ".") -> str:
    ...
    try:
        ...
        if tech_stack:
            explanation.extend(
                ["Technology Stack:", *[f"  • {tech}" for tech in tech_stack], ""]
            )
        ...
        if key_dirs:
            explanation.extend(
                [
                    ...
                    *[
                        f"  • {dir_name}/: {purpose}"
                        for dir_name, purpose in key_dirs.items()
                    ],
                    "",
                ]
            )
...
def get_project_context(path: str = ".") -> str:
    ...
    try:
        ...
        if "package.json" in [
            ...
            f.endswith(("requirements.txt", "pyproject.toml"))
            for f in structure["config_files"].keys()
        ):
