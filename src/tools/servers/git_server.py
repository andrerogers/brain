#!/usr/bin/env python3
"""
Local MCP Server for Git operations
Provides git status, diff, log, commit, and other version control capabilities
"""
...
@mcp.tool()
def git_diff(file_path: Optional[str] = None, staged: bool = False, path: str = ".") -> str:
    """
...
def git_commit(message: str, path: str = ".") -> str:
    ...
    commit_hash = hash_result["stdout"].strip()[:8] if hash_result["success"] else "unknown"
...
def git_branch_info(path: str = ".") -> str:
    ...
    remotes = remote_result["stdout"].strip() if remote_result["success"] else "No remotes configured"
    ...
    if ahead_behind_result["success"]:
        ...
        if len(counts) == 2:
            ...
            if ahead != "0" or behind != "0":
                output.append(f"Branch status: {ahead} ahead, {behind} behind origin/{current_branch}")
            else:
...
def git_search_history(query: str, limit: int = 20, path: str = ".") -> str:
    ...
    code_result = run_git_command([
        "git", "log", "-S", query, f"-{limit}", "--oneline", "--decorate"
    ], cwd=path)
