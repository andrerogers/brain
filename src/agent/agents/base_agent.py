...
class BaseAgent(ABC):
    """
    Base class for all Agent Engine agents.

    - Pydantic AI agent management
    - MCP tool access via bridge
    ...
    def __init__(
        ...
        # Tool info objects from app layer
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.available_tools = await self.tool_bridge.get_tools_for_agent_type(self.config.agent_type)

            self.logger.info(f"Agent initialized with {
    ...
    async def execute_with_progress(
        ...
        if self.is_busy:
            raise RuntimeError(
                f"{self.config.agent_type} agent is currently busy")
        ...
        try:
            ...
            self.logger.info(f"Agent execution completed in {
    ...
    async def execute_tool(
        ...
        try:
            ...
            is_valid, error_msg = await self.tool_bridge.validate_tool_parameters(tool_name, parameters)
            if not is_valid:
                ...
                if tool_info and hasattr(tool_info, 'parameters') and tool_info.parameters:
                    detailed_error += f"\n\nTool Schema Reference:\n{
                        self._format_single_tool_schema(tool_info)}"

                self.logger.error(
                    f"Tool parameter validation failed: {detailed_error}")
            ...
            if tool_call.error:
                ...
                if any(keyword in str(tool_call.error).lower() for keyword in
                       ['required', 'missing', 'parameter', 'argument', 'validation']):
                    tool_info = self.tool_bridge.get_tool_by_name(tool_name)
                    if tool_info:
    ...
    async def get_tool_recommendations(self, task: Task) -> List[str]:
        """Get tool recommendations for a task."""
        return await self.tool_bridge.get_tool_usage_recommendations(task, self.config.agent_type)

    def format_tools_for_prompt(self, tools: List[Any] = None) -> str:
    ...
    def _get_tool_usage_example(self, tool_name: str, properties: Dict[str, Any], required: List[str]) -> str:
    ...
    def _format_single_tool_schema(self, tool_info) -> str:
        ...
        lines.append(f"Description: {getattr(
        ...
        for param_name, param_info in properties.items():
            ...
            default_val = f", default: {
                param_info['default']}" if 'default' in param_info else ""
        ...
        # Add usage example
        example = self._get_tool_usage_example(
            tool_info.name, properties, required)
        if example:
            lines.append(f"Example: {example}")

        return "\n".join(lines)

    def create_tool_execution_context(self, **kwargs) -> Dict[str, Any]:
